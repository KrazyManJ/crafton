export function minecraftColorsToHTML(txt) {
    const COLORS = {
        "0": "#000000",
        "1": "#0000AA",
        "2": "#00AA00",
    ...
    const fct = (text, reg, prefix, map, f) => text.replace(reg, (_, code, val) => `<span style="${prefix}${map[code.substring(1)]}">${f ? f(val) : val}</span>`);
    ...
    if (options) {
        ...
        if (options.cls) {
            if (Array.isArray(options.cls))
                elem.classList.add(...options.cls);
            else
                elem.classList.add(options.cls);
        }
        if (options.attribs)
            Object.keys(options.attribs).forEach(k => elem.setAttribute(k, options.attribs[k]));
        if (options.css)
            Object.keys(options.css).forEach(k => elem.style.setProperty(k, options.css[k]));
        if (options.parent)
            options.parent.appendChild(elem);
        if (options.children)
            options.children.forEach(ch => elem.appendChild(ch));
    }
    return elem;
}
